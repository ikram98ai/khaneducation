...
class CRUDBase(Generic[ModelType]):  # Simplify generic signature
    ...
    def update(self, db_obj: ModelType, obj_in_data: dict) -> ModelType:  # Pass the model instance and update data
        ...
        try:
            ...
            logger.error(
                f"Error updating {self.model.Meta.table_name} with key {getattr(db_obj, db_obj.attribute_values.get(db_obj._hash_key_attribute().attr_name, 'unknown'))}: {e}"
            )
    ...
    def remove(self, db_obj: ModelType) -> ModelType:  # Pass the model instance to delete
        ...
        try:
            ...
            logger.error(
                f"Error deleting {self.model.Meta.table_name} with key {getattr(db_obj, db_obj.attribute_values.get(db_obj._hash_key_attribute().attr_name, 'unknown'))}: {e}"
            )
