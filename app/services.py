...
def create_lesson_with_content(db: Session, subject_id: int, instructor_id: int, title: str) -> models.Lesson:
    ...
    db_quiz = crud.crud_quiz.create(db, obj_in=schemas.QuizBase(lesson_id=db_lesson.id, version=1))
...
def submit_quiz_responses(db: Session, quiz_id: int, student_id: int, responses: List[Dict[str, Any]]) -> schemas.QuizAttempt:
    ...
    db_attempt = crud.crud_quiz_attempt.create(db, obj_in=attempt_in)
    ...
    if score < 60 and quiz.ai_generated:
        lesson = quiz.lesson
        new_version = max([q.version for q in lesson.quizzes]) + 1
